...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 29 22:28:21 2020
...
def causas_muerte_pais_anno(fichero):
    """
    ...
    Preconditions
    -------------
    El fichero debe estar situado en la ruta indicada
    
    El fichero NO debe estar vacio
    
    El fichero debe estar en formato .csv
    
    Example
    -------
    >>> causas_muerte_pais_anno('annual-number-of-deaths-by-cause.csv')
...
def eliminar_caracter(columna, caracter):
    """
    Funcion que elimina un caracter de una columna, 
    en formato de expresion regular, y su posterior
    conversion a formato float

    Parameters
    ----------
    columna : pandas.DataFrame
        Columna sobre la que aplicar la funcion
    caracter : str
        Expresion regular con la que sustituir
...
def renombrar_columnas(columnas):
    """
    Funcion encargada de renombrar 
    las columnas, empleando una
    expresion regular que simplifica
    el nombre de cada columna entre
    guiones
    
    Parameters
    ----------
    columnas : list
        Lista con los nombres originales
        de columnas

    Returns
    -------
    list
        Lista con el nombre de cada 
        columna simplificado
    ...
    return list(map(lambda column : re.search(patron, column)[0].strip() if '-' in column else column, columnas))
        
def filtrar_tabla(df_mortalidad, nombre_columna, parametros):
    """
    Metodo que permite filtrar los valores del DataFrame
    en funcion de la columna proporcionada y el conjunto
    de parametros.
    ...
    Parameters
    ----------
    df_mortalidad : pandas.DataFrame
        Tabla de datos con 
        los valores de mortalidad
    nombre_columna : str
        Nombre de la columna sobre
        la que aplicar el filtro
    parametros : set
        Conjunto de parametros con
        los que realizar el filtro
    ...
    El campo nombre_columna debe estar en el DataFrame
    
    El campo parametros NO debe estar vacio ({})

    Example
    -------
    >>> filtrar_tabla(df_mortalidad, "Year", {1998, 1999})
           Entity  Year   ... - Interpersonal violence -
    ...
    elif nombre_columna not in df_mortalidad.columns:
        raise KeyError("Error. No se ha encontrado el nombre de la columna especificada")
    elif parametros == {}:
        raise ValueError("Error. El campo parametros esta vacio ({})")
        
    return df_mortalidad[list(map(lambda fila: fila in parametros, df_mortalidad[nombre_columna]))]
    

def evolucion_mortalidad(df_mortalidad):
    """
    Funcion que permite calcular
    la evolucion de la mortalidad
    en un pais y anno
    ...
    Los valores del DataFrame DEBEN ser
    numericos
    
    Example
    -------
    >>> evolucion_mortalidad(tabla_filtrada)
    ...
    if df_mortalidad.empty:
        raise ValueError("Error. El DataFrame esta vacio")
    try:
        return df_mortalidad.diff().fillna(0)
